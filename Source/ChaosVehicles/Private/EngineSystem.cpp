...
namespace ChaosTracked
{
	...
	FTrackedEngineSim::FTrackedEngineSim(const FTrackedEngineConfig* StaticDataIn) : TVehicleSystem<FTrackedEngineConfig>(StaticDataIn)
		, MaxTorque(Setup().MaxTorque)
		, ThrottlePosition(0.f)
		, TargetSpeed(0.f)
		, CurrentRPM(0.f)
		, DriveTorque(0.f)
		, EngineIdleSpeed(Chaos::RPMToOmega(Setup().EngineIdleRPM))
		, MaxEngineSpeed(Chaos::RPMToOmega(Setup().MaxRPM))
		, EngineStarted(true)
		, FreeRunning(false)
		, Omega(0.f)
		, RevRate(0.)
	...
	float FTrackedEngineSim::GetTorqueFromRPM(float RPM, bool LimitToIdle /*= true*/)
	{
	...
	void FTrackedEngineSim::Simulate(float DeltaTime)
	{
		if (!EngineStarted)
		{
			return;
		...
		if (FreeRunning)
		{
			...
			float Decel = Setup().EngineRevDownRate * Chaos::Sqr((Omega - 0.5f*EngineIdleSpeed) / MaxEngineSpeed);
		...
		// EngineSpeed == Omega
		CurrentRPM = Chaos::OmegaToRPM(Omega);
	}
