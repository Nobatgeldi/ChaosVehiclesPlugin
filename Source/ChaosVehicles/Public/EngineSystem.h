...
namespace ChaosTracked
{
	...
	struct FTrackedEngineConfig
	{
		FTrackedEngineConfig()
			: MaxTorque(0.f)
			, MaxRPM(6000)
			, EngineIdleRPM(1200)
	...
	class FTrackedEngineSim : public TVehicleSystem<FTrackedEngineConfig>
	{
	public:

		FTrackedEngineSim(const FTrackedEngineConfig* StaticDataIn);
		...
		{
			Chaos::FVehicleUtility::ClampNormalRange(InThrottle);
		...
		void SetEngineRPM(bool FreeRunningIn, float InEngineRPM)
		{
			...
			if (!FreeRunning)
			{
				TargetSpeed = Chaos::RPMToOmega(FMath::Clamp(FMath::Abs(InEngineRPM), (float)Setup().EngineIdleRPM, (float)Setup().MaxRPM));
			}
		}
